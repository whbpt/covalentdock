/*
 ...
 $Id: main.cpp,v 1.72 2009/09/01 23:38:29 rhuey Exp $
...
/*-----------------------------CovalentDock-----------------------------*/
            for (indx_r = 1;  indx_r < MAX_DIST;  indx_r++) {
				...
				else if ((strcmp(gridmap[ia].type,"V1")==0)&&((i==get_rec_index("LA"))||(i==get_rec_index("L"))))
				{
					double d=r-1.83606;
					double penalty=0;
					penalty=10.1626*(exp(-2*3.4055*d)-2*exp(-3.4055*d))+4.6202;
					if ((d>0) && (penalty>0)) penalty=0;
					energy_lookup[i][indx_r][ia] = min(EINTCLAMP, penalty);
					do_smooth=false;
				}
				else if ((strcmp(gridmap[ia].type,"V2")==0)&&((i==get_rec_index("YA"))||(i==get_rec_index("YS"))))
				{
					double d=r-1.35852;
					double penalty=0;
					penalty=21.5449*(exp(-2*3.7758*d)-2*exp(-3.7758*d))+10.9614;
					if ((d>0) && (penalty>0)) penalty=0;
					energy_lookup[i][indx_r][ia] = min(EINTCLAMP, penalty);
					do_smooth=false;
				}
...
/* END NEW2 calculate dot product of bond vector with bond vector of best hbond */
						...
						atom_type[ia]==get_rec_index("YA") || atom_type[ia]==get_rec_index("YS"))
				{
					...
					//c[XYZ] is the current grid point
					//carb[XYZ] is the coordinates of beta
					double carb[XYZ];
					carb[0]=coord[beta][0];
					carb[1]=coord[beta][1];
					carb[2]=coord[beta][2];
					...
					for (i=0;i<XYZ;i++)
					{
						V1[i]=carb[i]-coord[ia][i];
						V2[i]=c[i]-coord[ia][i];
					}

					double len1=hypotenuse( V1[X], V1[Y], V1[Z]);
					double len2=hypotenuse( V2[X], V2[Y], V2[Z]);

					double prod=V1[0]*V2[0]+V1[1]*V2[1]+V1[2]*V2[2];
			...
			//	coef=1;
                ...
                for (map_index = 0;  map_index < num_atom_maps;  map_index++) {
                    ...
                    if (gridmap[map_index].is_covalent == FALSE) {
						...
						else if (gridmap[map_index].is_hbonder == TRUE) {
